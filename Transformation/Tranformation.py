...
class Timeplan:
    ...
    def student_to_dict(self,prename,surname,sclass,eventlist):
        temp = {
            ...
            'surname': surname,
            'schoolClass': sclass,
            'wishlist': [{'wishId': wish.getCompID(), 'timeslot': wish.getTimeslot()} for wish in eventlist]
        }
        return temp
    def togo_list_to_json(self, studentlist):
        ...
        return json.dumps({'students':student_list}, indent=4, ensure_ascii=False)
...
start = Transform(response.json,response1.json)
